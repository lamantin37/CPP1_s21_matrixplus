...
#define NOT_SQUARED_MATRIX()                                                  \
  do {                                                                        \
    (_rows != _cols) ? throw invalid_argument(                                \
                           "Matrix must be square to calculate determinant.") \
                     : 0;                                                     \
  } while (0)
#define ZERO_DET()                                                        \
  do {                                                                    \
    (det == 0.0) ? throw invalid_argument(                                \
                       "Matrix is not invertible (determinant is zero).") \
                 : 0;                                                     \
  } while (0)
#define INVALID_SETTER(m)                                                 \
  do {                                                                    \
    (m < 0) ? throw invalid_argument("Error: wrong setter argument") : 0; \
  } while (0)
...
#define MATRIX_OPERATION(op, m)                                \
  do {                                                         \
    ...
    for (int i = 0; i < _rows * _cols; ++i)                    \
      result._matrix[i / _cols][i % _cols] =                   \
          _matrix[i / _cols][i % _cols] op OPERATION_TYPE_##m; \
    return result;                                             \
  } while (0)

#define ASSIGNMENT_OPERATORS(m) \
  do {                          \
    m##_matrix(other);          \
    return *this;               \
  } while (0)
...
class S21Matrix {
 ...
 public:
  ...
  inline void ALLOCATE_MEMORY() {
    this->_matrix.resize(this->_rows, vector<double>(this->_cols));
  }
