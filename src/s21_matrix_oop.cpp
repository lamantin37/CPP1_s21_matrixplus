...
S21Matrix::S21Matrix(int x, int y) : _rows(x), _cols(y) {
  (x < 0 || y < 0) ? throw out_of_range("Error: out of range")
                   : this->ALLOCATE_MEMORY();
}
...
S21Matrix S21Matrix::operator+(const S21Matrix& other) const {
  DIFFERENT_SIZES();
  MATRIX_OPERATION(+, BASIC_ARITHMETIC);
}
S21Matrix S21Matrix::operator-(const S21Matrix& other) const {
  DIFFERENT_SIZES();
  MATRIX_OPERATION(-, BASIC_ARITHMETIC);
}
S21Matrix S21Matrix::operator*(double scalar) const {
  MATRIX_OPERATION(*, SCALAR);
}
...
S21Matrix& S21Matrix::operator+=(const S21Matrix& other) {
  ASSIGNMENT_OPERATORS(sum);
}
S21Matrix& S21Matrix::operator-=(const S21Matrix& other) {
  ASSIGNMENT_OPERATORS(sub);
}
S21Matrix& S21Matrix::operator*=(const S21Matrix& other) {
  ASSIGNMENT_OPERATORS(mul);
}
...
S21Matrix S21Matrix::operator*(const S21Matrix& other) const {
  WRONG_DIMENSIONS();
  S21Matrix result(this->_cols, other._rows);
